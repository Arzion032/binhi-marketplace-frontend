...
console.log(product)
  ...
  return (
    <div className=" flex flex-col bg-[#F5F9F5] ">
      {showToast && (
      ...
      <div className="px-6 py-2"> 
        <div className="pl-10 flex items-center mt-5">
      ...
      <div className="pl-10 pr-6 py-8">
        <div className="pl-10 grid grid-cols-1 md:grid-cols-[minmax(0,400px)_1fr] gap-x-10">
            ...
            {product && (
              [
                ...
                <div
                  ...
                  className={`w-16 h-16 border-2 rounded-md cursor-pointer overflow-hidden hover:bg-green-100 transition-transform duration-400 hover:scale-125 ${
          ...
          {/* <div className="text-center text-base text-gray-800 mb-6">
          ...
          <div>
            ...
            <div className="mt-4">
              <div className="flex items-center">
                <span className="text-4xl font-bold text-[#4CAE4F]">₱ {price}</span>
                <span className="ml-2 px-2 py-1 bg-white border border-[#4CAE4F] font-semibold text-base text-[#4CAE4F] rounded">{unit}</span>
                <span className="ml-1 text-lg text-gray-500 line-through">{product.originalPrice}</span>
              </div>
            </div>

            <div className="mt-4 text-gray-700 text-lg">{product.description}</div>
            <div className="mt-4"><span className="font-bold text-lg">Category:</span> <span className=" text-lg pl-5">{product.category_name}</span></div>

            <div className="mt-6">
          <div className="flex mt-2 gap-4 flex-wrap">
            <span className="font-bold text-lg pr-3">Variation:</span>
          ...
          {showVariationError && (
            ...
            <div className="mt-6 flex items-center">
              <span className="font-bold text-lg w-24">Quantity:</span>
                <div className="flex items-center overflow-hidden pl-2">
                  <button
                    type="button"
                    onClick={decrementQuantity}
                    className="disabled:opacity-50 cursor-pointer transition-transform duration-400 hover:scale-105"
                  >
                  ...
                  <button
                    ...
                    className="disabled:opacity-50 hover:bg-green-100 transition-transform duration-400 hover:scale-105"
                  >
          ...
          <button
            ...
            className="border-2 border-[#4CAE4F] text-xl font-medium text-[#4CAE4F] hover:bg-green-100 transition-transform duration-400 hover:scale-95 px-6 py-2 rounded-full w-[350px] h-[60px] flex items-center justify-center gap-2"
          >
          ...
          <button
            ...
            className="bg-[#4CAE4F] text-white text-xl hover:bg-green-600 transition-transform duration-400 hover:scale-95 font-bold px-6 py-6 rounded-full flex-1 h-[60px] max-w-[930px] flex items-center justify-center gap-2"
          >
            Buy Now
        ...
        {/* Seller Info - Full width, aligned with both columns */}
        <div className="pl-10">
          <VendorDetails selectedFilter={selectedFilter} product={product} tags={tags} />     
        </div>

        {/* Recommendations - Full width, aligned with both columns */}
        <div className="mx-10 pl-10 mt-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {[
              ...
              >
                <span className="w-[130px] bg-green-100 text-green-800 text-sm text-center font-semibold px-2 py-1 rounded-full mb-2">
                  VEGETABLE
                </span>
                <img src={product.image} alt={product.name} className="w-full h-40 object-contain rounded-xl" />
                <p className="text-left font-semibold text-lg mt-2">{product.name}</p>
                <p className="text-[#4CAE4F] text-xl font-bold">
                  {product.price}{' '}
                </p>
                <div className="text-[20px] text-gray-600 mt-2 flex items-center gap-1">
                  <img src="/Star.png" alt="star" className="w-4 h-4" />
                   {product.sold} Sold
                </div>
                <div className="flex items-center justify-between mt-2">
                  <img src="/shopping-cart.png" alt="cart" className="w-8 h-8 transition-transform duration-100 hover:scale-125" />
                  <button
                    onClick={() => {
                    ...
                    className="bg-[#4CAE4F] text-white hover:bg-green-600 font-bold px-2 py-2 rounded-full w-[200px] text-center items-center"
                  >
                    Buy Now
                  </button>
                </div>
          ...
          <div className="flex justify-center mt-10">
            <button className="text-lg font-bold bg-white border-2 hover:border-[#4CAE4F] border-[#4CAE4F] text-[#4CAE4F] hover:bg-green-100 w-[500px] h-[50px] px-4 py-1 rounded-full text-center transition-transform duration-100 hover:scale-110">
              See More
            </button>
          </div> 
        </div>
