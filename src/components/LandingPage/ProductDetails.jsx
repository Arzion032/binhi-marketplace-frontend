...
const ProductDetails = () => {
  ...
  useEffect(() => {
    api.get(`/products/detail/${productSlug}/`)
      .then(res => {
        ...
        // Set initial variation and price
        if (productData.variations && productData.variations.length > 0) {
          const defaultVariation = productData.variations.find(v => v.is_default) || productData.variations[0];
          setSelectedVariation(defaultVariation);
          setPrice(defaultVariation.unit_price);
          setUnit(defaultVariation.unit_measurement);
          
          // Set initial main image
          const mainImg = defaultVariation.images?.find(img => img.is_main) || 
                         defaultVariation.images?.[0] || 
                         productData.images?.find(img => img.is_main) || 
  ...
  const handleVariationSelect = (variation) => {
    ...
    setPrice(variation.unit_price);
    setUnit(variation.unit_measurement);
    
    // Update main image
    const newMainImage = variation.images?.find(img => img.is_main) || 
                        variation.images?.[0] || 
            ...
            <div className="mt-6">
              <div className="flex mt-2 gap-4 flex-wrap">
                ...
                {product.variations && product.variations.map((variation, index) => {
                  const mainImageObj = variation.images?.find(img => img.is_main) || variation.images?.[0];
                  ...
                  return (
                    <div
                      ...
                      className={`border-2 bg-white rounded-xl px-4 py-1 flex items-center cursor-pointer transition ${
                        selectedVariation && selectedVariation.id === variation.id ? 'border-green-500' : 'border-gray-400'
                      }`}
                    >
                      {mainImageObj ? (
                        <img
                    ...
                    items: [
                      {
                        ...
                        variation: selectedVariation?.name || 'Default',
                        seller: product.seller?.name || 'Unknown Seller',
                        orderId: `PD-${Date.now()}`,
                        weight: calculateItemWeight(),
                        unit: selectedVariation?.unit_measurement || unit || 'pc'
                      },
                    ],
                    subtotal: quantity * parseFloat(price.toString().replace(/[₱,]/g, "")),
                    discount: 0,
                    tax: 0,
                    total: quantity * parseFloat(price.toString().replace(/[₱,]/g, "")),
